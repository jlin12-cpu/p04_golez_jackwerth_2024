"""
Extended Figure 3 from Golez & Jackwerth (2024).

Reproduces Figure 3 (annualized standard deviation across holding periods)
using all available months in _data/calc/monthly_returns.parquet, starting
in 1996 and extending beyond the paper's original end date (2022-12-31)
whenever newer data are present.

- Panel A: Returns in excess of the risk-free rate
- Panel B: Returns in excess of Treasury bond returns

For each holding period h in [1, 6, 12, 18, 24, 30, 36] months, overlapping
h-period log returns are formed by summing h consecutive monthly log returns,
then annualized as:

    annualized_std = std(h-period return) / sqrt(h) * sqrt(12)

This figure examines whether the paper's main volatility pattern --
a sharp decline in strip volatility with holding period and a more stable
market volatility profile -- continues to hold in the extended sample.
...
OUTPUT_DIR = Path("output/figure3_extended")
...
def plot_figure3_extended(df_stds: pd.DataFrame, end_date) -> None:
    """
    Plot extended Figure 3 with two vertically stacked panels.
    ...
    Parameters
    ----------
    df_stds  : pd.DataFrame  annualized std series from calc_all_stds()
    end_date : date          latest date in the sample, used in title
    """
    # Dynamic y-axis: round up to nearest 5, add 5 for headroom
    all_vals = df_stds[["std_strip_rf", "std_mkt_rf",
                         "std_strip_2y", "std_mkt_10y"]]
    y_max = int(np.ceil(all_vals.max().max() / 5)) * 5 + 5
    y_min = max(0, int(np.floor(all_vals.min().min() / 5)) * 5 - 5)
    ...
    out_path = OUTPUT_DIR / "figure3_extended.png"
...
if __name__ == "__main__":
    print("=" * 60)
    print("PLOTTING FIGURE 3 (EXTENDED SAMPLE)")
    print("=" * 60)

    df = load_data()
    end_date = df["date"].max().date()
    print(f"Extended sample through: {end_date}")

    df_stds = calc_all_stds(df)
    plot_figure3_extended(df_stds, end_date)

    out_csv = OUTPUT_DIR / "figure3_extended_series.csv"
